...
export function HomePage({ locale = 'it' }: { locale?: Locale }) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b border-white/8 bg-[#080b14]/88 backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-[1480px] flex-wrap gap-3 py-3 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2" aria-label={t("Navigazione principale")}>
            <LanguageSwitcher path="/" locale={locale} />
            ...
            <a href={MAC_DMG_DOWNLOAD_URL} download={MAC_DMG_FILENAME} title={t(MAC_DMG_DESCRIPTION)} className="brand-button inline-flex h-9 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-white sm:px-4">
              <Download className="size-4" />
              <span className="hidden sm:inline">{t("Scarica per Mac")}</span>
          ...
          <div className="mb-7 flex flex-col items-start justify-between gap-5 lg:flex-row lg:items-end">
            <div className="max-w-4xl">
              <span className="mb-4 inline-flex h-6 items-center gap-1.5 rounded-full border border-cyan-300/20 bg-cyan-300/8 px-2.5 text-xs font-semibold text-cyan-200">
                <ShieldCheck /> {t("Nessun account · Nessun caricamento")} </span>
              <h1 className="max-w-4xl text-balance text-4xl font-black tracking-[-0.045em] text-white sm:text-5xl lg:text-[58px] lg:leading-[1.03]"> {t("Lavora sui tuoi PDF.")} <span className="brand-gradient-text block">{t("Gratis, privato, nel browser.")}</span>
              </h1>
              <p className="mt-4 max-w-3xl text-pretty text-base leading-7 text-slate-300 sm:text-lg"> {t("Aggiungi testo, comprimi, dividi e converti in Word. Puoi anche coprire e riscrivere le scritte: è una modifica visiva, non una cancellazione dell’originale. Il documento viene elaborato sul tuo dispositivo, senza upload.")} </p>
            </div>

            <div className="grid grid-cols-2 gap-3 text-sm lg:w-[370px]">
              <TrustMetric icon={<LockKeyhole />} label={t("Elaborazione")} value={t("Locale")} />
              <TrustMetric icon={<GitFork />} label={t("Codice")} value={t("Pubblico")} />
            </div>
          </div>

          <PdfEditor locale={locale} />

          <nav aria-label={t("Strumenti PDF")} className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Capability href={path("/modifica-pdf")} icon={<PencilLine />} title={t("Modifica visiva")} text={t("Copri e riscrivi con un font sostitutivo. L’originale resta recuperabile: non è redazione sicura.")} />
            <Capability href={path("/comprimi-pdf")} icon={<FileArchive />} title={t("Comprimi PDF")} text={t("Ottimizzazione senza perdita oppure riduzione forte del peso.")} />
            <Capability href={path("/dividi-pdf")} icon={<Scissors />} title={t("Dividi pagine")} text={t("Estrai un intervallo o crea uno ZIP con le singole pagine.")} />
      ...
      <section id="privacy" className="border-y border-white/8 bg-white/[.018] px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-[1200px] gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          <div>
            <p className="eyebrow">{t("Semplice per scelta")}</p>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-4xl">{t("Niente account. Niente abbonamento.")}</h2>
            <p className="mt-4 max-w-xl text-sm leading-6 text-slate-400">{t("La versione web non invia il contenuto dei PDF a Tomorrow Now. Il servizio di hosting può comunque trattare i normali dati tecnici di accesso, descritti nell’informativa privacy.")}</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <FeatureCard icon={<LockKeyhole />} title={t("File locale")} text={t("Il documento resta nella memoria del browser.")} />
            <FeatureCard icon={<ShieldCheck />} title={t("Trasparente")} text={t("Codice e licenze consultabili pubblicamente.")} />
            <FeatureCard icon={<ArrowRight />} title={t("Subito utile")} text={t("Apri il PDF e comincia senza registrarti.")} />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1200px]">
          <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-4xl">{t("Parti online. Continua con gli strumenti avanzati su Mac.")}</h2>
            ...
            <div className="rounded-[28px] border border-white/9 bg-[#0b0f1a] p-6 sm:p-8">
              ...
              <h3 className="mt-5 text-xl font-bold text-white">{t("Software libero, marchio riconoscibile")}</h3>
              <p className="mt-3 text-sm leading-7 text-slate-400">{t("Il codice dell’app Mac è pubblicato con licenza GNU AGPL v3. Il nome e i loghi Tomorrow Now restano marchi distintivi e non autorizzano build modificate a presentarsi come ufficiali.")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 pb-8 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black tracking-tight text-white sm:text-3xl">{t("Software che semplifica il lavoro di domani.")}</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">{t("Scopri gli altri prodotti, automazioni e soluzioni digitali progettati da Tomorrow Now.")}</p>
          </div>
          <span className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/10 px-5 text-sm font-bold text-white transition group-hover:border-cyan-200/40 group-hover:bg-white/15"> {t("Visita Tomorrow Now")} <ExternalLink className="size-4" />
          </span>
        </a>
      </section>
      ...
      <footer id="open-source" className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1480px] flex-col gap-5 border-t border-white/8 pt-8 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          <p>{t("© 2026 Tomorrow Now. Tomorrow Now PDF Editor è software open source.")}</p>
